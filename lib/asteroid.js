(function () {
  ...
  var Asteroid = Asteroids.Asteroid = function(pos, game, radius, imgSrc, level) {
    ...
    console.log(this.radius, this.level, this.img.height);
  }
    ...
    if(otherObject instanceof Asteroids.Ship) {
      ...
      if (this.level < 3) {
        for (var i = 0; i < 2; i++) {
          ...
          newPos = [(this.pos[0] + newVec[0] + 800) % 800, (this.pos[1] + newVec[1] + 600) % 600];
          this.game.addAsteroid(new Asteroid(newPos, game, this.radius / 2, "vendor/img/asteroid" + (this.level + 1) + ".1.png", this.level + 1));
        }
      }
