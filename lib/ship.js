(function () {
  ...
  Ship.prototype.shoot = function(type) {
    ...
    if (this.pulseCannon) {
      for (i = 0; i <= this.pulseCannon; i++) {
        var degRot = Asteroids.Util.degToRad(this.rotation + (90 * i));
        var bulletPos = [this.pos[0] + Math.cos(degRot) * 15 * i,
                         this.pos[1] - Math.sin(degRot) * 15 * i];
