(function () {
  ...
  Ship.prototype.power = function(inc) {
    ...
    this.vel[0] += this.impulse()[0] * 0.5;
    this.vel[1] += this.impulse()[1] * 0.5;
  ...
  Ship.prototype.shoot = function(type) {
    ...
    if (this.pulseCannon) {
      ...
      for (i = 1; i < this.pulseCannon; i++) {
        var degRot = Asteroids.Util.degToRad(this.rotation);
        var degDistortion = [0, 180, 90, 270][i]
        this.addBullet({type: type, rot: this.rotation + degDistortion});
        this.timeSinceBulletFired -= 5;
      }
    }

    if (this.multiCannon) {
      var degDistortion = (this.multiCannon) * 10;
      for (var i = -degDistortion; i < degDistortion+1; i += 10) {
